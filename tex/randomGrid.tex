...
Estimates of the simulated life history parameters obtained from Fishbase are presented in Fig.1. These show that for fast growing species as short lived small in size $l_{\infty}$ (asymtopic length parameter) species such as sprat, the growth parameter \textit{k} is high and its age-at-50\%-maturity \textit{a50} are low, in contrast to a slower growing larger $l_{\infty}$ (high asymtopic length parameter) longer lived species such as rays or pollack.
...
Fig.3 displays the simulated time series resulting from the operating model in relation to reference points by species with the trends in annual stock estimates in relation to (bmsy). 
