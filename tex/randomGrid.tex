...
Here we describe methods to assess the performance of each empirical HCR via a set of utilities and show the benefits compared to a constant catch strategy across a spectrum of different species: safety (recruitment relative to virgin recruitment), yield ($yield/MSY$), kobe proportion (proportion of years that stay in the green zone of kobe plot ($B/B_{MSY} >1$ and $F/F_{MSY} <1$), and Yield Annual Variation (Average annual variation in a TAC from one year to the next (expressed as a proportion of the average annual catch). 
